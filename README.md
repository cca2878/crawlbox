...
Named volumes hold manager configuration/catalog, the Kopia repository and server secrets, the shared connection handoff, and installed extensions. No Compose `user` setting is required: the manager entrypoint prepares its mount root then drops to UID/GID 10001; the Kopia wrapper retains the official image's default user. Keep these volumes when recreating containers. First-start preparation preserves existing credentials and configuration; restarting does not reset the administrator. See [NAS deployment and operations](docs/nas.md).
