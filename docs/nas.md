...
New installations generate block-style YAML for manual editing. Existing configurations, including JSON-formatted files generated by older releases, are preserved on restart. To use YAML examples with an older JSON-formatted file, first convert the entire document to YAML while retaining its values; do not paste block-style YAML inside a JSON object.
