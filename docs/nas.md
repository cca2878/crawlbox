...
The Docker image provides `KOPIA_UI_PROXY_LISTEN=:8081`, `KOPIA_UI_PROXY_TARGET=https://kopia:51515` and `KOPIA_UI_PROXY_PIN_FILE=/bootstrap/connection.json`. These are independent of the business `KOPIA_URL` connection. Change the proxy target separately if you rename the storage service. YAML `kopia_ui_proxy` fields in `deploy/config.example.yaml` take precedence over these defaults. For direct binary deployments no proxy listener is created unless configured; an empty fingerprint file setting uses standard system CA verification for HTTPS. The manual Compose clears the image proxy defaults so its existing connection layout remains usable; configure the optional listener and add a host port mapping there when desired.
