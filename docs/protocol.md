...
Result may include an optional `message` (UTF-8 plain text, at most 4096 bytes). The host rejects oversized messages. It displays the message as a supplement to its own final outcome only for successful publication or `no_change`; validation, snapshot, commit failure or interruption must not display a success message. Omitted/blank messages retain the default outcome. Messages are kept in management run history, not Plugin State or business snapshots, and are not logged. Recovery may therefore display the default outcome if the final run record was not saved. This additive ABI v1 field is optional for old plugins; older hosts ignore it.
